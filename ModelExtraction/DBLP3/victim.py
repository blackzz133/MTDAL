...
def DBLP3_victim_model(args, victim_type):
...
def objective_function(model, dataset, device):
    ...
    for time, snapshot in enumerate(dataset):
        with torch.cuda.device(device=device):
            ...
            victim_labels = torch.argmax(model.to(device)(x, edge_index, edge_attr).detach(), dim=1).long().clone().to(
                device)
...
# 0.8532 lr=0.002 DCRNN
#The accuracy result of rgcn is tensor(0.7029, device='cuda:0')
#The accuracy result of rgcn is tensor(0.6897, device='cuda:0')

# 0.8866 lr=0.008 EVOLVE
#The accuracy result of rgcn is tensor(0.6758, device='cuda:0')
#The accuracy result of rgcn is tensor(0.6646, device='cuda:0')

#0.8789  lr=0.008 TGCN
#The accuracy result of rgcn is tensor(0.6724, device='cuda:0')
#The accuracy result of rgcn is tensor(0.6562, device='cuda:0')

#0.8731 lr=0.008 A3TGCN
# The accuracy result of rgcn is tensor(0.6750, device='cuda:0')
# The accuracy result of rgcn is tensor(0.6607, device='cuda:0')
