...
def DBLP_attack_model(args, victim_model, dataset, attack_type, device, with_cost):
    loader1 = DBLPLoader('DBLP5')
    ...
    optimizer_rgcn = torch.optim.Adam(model_rgcn.parameters(), lr=0.006)
    optimizer_rgcn2 = torch.optim.Adam(model_rgcn2.parameters(), lr=0.006)
    optimizer_rgcn3 = torch.optim.Adam(model_rgcn3.parameters(), lr=0.006)
    optimizer_rgcn4 = torch.optim.Adam(model_rgcn4.parameters(), lr=0.006)
    optimizer_gcn = torch.optim.Adam(model_gcn.parameters(), lr=0.006)
...
def rep(model, dataloaders):
    ...
    Rep = torch.tensor(
        [(Rep[i] - min_rep[cluster_ids_x[i]]) / (max_rep[cluster_ids_x[i]] - min_rep[cluster_ids_x[i]]) for j in
         range(Rep.shape[0])])
