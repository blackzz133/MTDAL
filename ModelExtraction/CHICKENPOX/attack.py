...
def Chickenpox_attack_model(args, victim_model, attack_type, device):
    loader1 = EnglandCovidDatasetLoader()
    loader2 = ChickenpoxDatasetLoader()
    ...
    print(torch.tensor(sim_trainloader.features).shape)
    print(torch.tensor(sim_testloader.features).shape)
    print(torch.tensor(raw_trainloader.features).shape)
    print(torch.tensor(raw_testloader.features).shape)
    ...
    for time, snapshot in enumerate(sim_trainloader):
        ...
        labels = victim_model(x, edge_index, edge_weight).detach().long().tolist()
    ...
    #print(torch.tensor(raw_dataset.targets).shape)
