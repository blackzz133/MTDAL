...
def DBLP3_attack_model(args, victim_model, dataset, attack_type, device, with_cost):
    loader1 = DBLPLoader('DBLP3')
    loader2 = DBLPLoader(dataset)
    ...
    '''
    print(torch.tensor(sim_dataset.features).shape)
    print(torch.tensor(sim_dataset.targets).shape)
    #print(torch.tensor(raw_dataset.features).shape)
    ...
    #raw_dataset = node_data_sampling(raw_dataset, sampling_num=2000)
    print("attack dataset is sampled")
    ...
    #get victim_labels for achieve node importance
    victim_labels = []
    ...
    # print(torch.tensor(sim_trainloader.edge_indices).shape)
    # print(torch.tensor(sim_trainloader.edge_weights).shape)
    #exit()
    ...
    lr = 1e-3
    optimizer_rgcn = torch.optim.Adam(model_rgcn.parameters(), lr=lr) #0.006
    optimizer_rgcn2 = torch.optim.Adam(model_rgcn2.parameters(), lr=lr)
    optimizer_rgcn3 = torch.optim.Adam(model_rgcn3.parameters(), lr=lr)
    optimizer_rgcn4 = torch.optim.Adam(model_rgcn4.parameters(), lr=lr)
    ...
    #存储test loss
    all_loss = {'RGCN': 0, 'RGCN2': 0, 'RGCN3': 0, 'RGCN4': 0, 'GCN': 0}
    all_loss2 = {'RGCN': 0, 'RGCN2': 0, 'RGCN3': 0, 'RGCN4': 0,'GCN': 0}
    all_loss3 = {'RGCN': 0, 'RGCN2': 0, 'RGCN3': 0, 'RGCN4': 0,'GCN': 0}
    #device
    ...
    all_loss['RGCN'], all_loss['RGCN2'], all_loss['RGCN3'], all_loss['RGCN4'], all_loss['GCN'] = objective_function(models, raw_testloader, device)
    all_loss2['RGCN'], all_loss2['RGCN2'], all_loss2['RGCN3'], all_loss2['RGCN4'], all_loss2['GCN'] = objective_function(models, raw_trainloader, device)
    all_loss3['RGCN'], all_loss3['RGCN2'], all_loss3['RGCN3'], all_loss3['RGCN4'], all_loss3[
        'GCN'] = objective_function(models, sim_trainloader, device)
    ...
    all_loss2['RGCN'], all_loss2['RGCN2'], all_loss2['RGCN3'], all_loss2['RGCN4'], all_loss2[
        'GCN'] = objective_function(models, raw_trainloader, device)
    all_loss3['RGCN'], all_loss3['RGCN2'], all_loss3['RGCN3'], all_loss3['RGCN4'], all_loss3[
        'GCN'] = objective_function(models, sim_trainloader, device)
    ...
    for cycle in range(time_span, cycles+1):
        ...
        if attack_type == 'MTTAL':
            new_lr = new_lr - lr / 10
            optimizers['optimizer_rgcn'] = torch.optim.Adam(models['RGCN'].parameters(), lr=new_lr)
            optimizers['optimizer_gcn'] = torch.optim.Adam(models['GCN'].parameters(), lr=new_lr)
            optimizers['optimizer_rgcn2'] = torch.optim.Adam(models['RGCN2'].parameters(), lr=new_lr)
            optimizers['optimizer_rgcn3'] = torch.optim.Adam(models['RGCN3'].parameters(), lr=new_lr)
            optimizers['optimizer_rgcn4'] = torch.optim.Adam(models['RGCN4'].parameters(), lr=new_lr)
        '''
        ...
        if attack_type =='ALTG':
            ...
            #new_models.pop('GCN')
            labeled_set = query_samples(models, attack_type, dataloaders, labeled_set, cycle, all_loss, args, q_cost, victim_labels)
        #without neighbor consideration
        if attack_type == 'ALTG2':
                # new_models = cp.deepcopy(models)
                # new_models.pop('GCN')
            labeled_set = query_samples(models, attack_type, dataloaders, labeled_set, cycle, all_loss, args,
                                            q_cost, victim_labels)
        if attack_type == 'M_Kcenter':
            labeled_set = query_samples(models, attack_type, dataloaders, labeled_set, cycle, all_loss, args,
                                        q_cost, victim_labels)
        ...
        all_loss2['RGCN'], all_loss2['RGCN2'], all_loss2['RGCN3'], all_loss2['RGCN4'], all_loss2['GCN'] = objective_function(
