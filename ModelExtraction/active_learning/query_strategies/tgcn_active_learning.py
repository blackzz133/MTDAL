...
class MTTAL_strategy:
    ...
    def query2(self, n):
        ...
        t_labeled_set = lset.reshape(lset.shape[0] * lset.shape[1]).numpy()
        qcost = t_query_cost[~t_labeled_set]
        used_budget = np.sum(t_query_cost[t_labeled_set])

        cost_in_time = {time:np.sum(t_query_cost[time*self.node_num:(time+1)*self.node_num][t_labeled_set[time*self.node_num:(time+1)*self.node_num]]) for time in range(self.time_span)}
        #standard_qcost = qcost/max(qcost)

        #get standard_qcost
        q2 = sorted(np.unique(t_query_cost))
        # print(s2)
        q2_ = {float(q2[i]): i for i in range(len(q2))}
        new_q = [q2_[float(t_query_cost[i])]+ 1 for i in range(lset.shape[0] * lset.shape[1])]
        new_q_ = np.array(new_q) / max(new_q)
        standard_qcost = new_q_[~t_labeled_set]

        qcost_min = np.array(qcost).min()
        qcost_max = np.array(qcost).max()
        # the weight of representative
        alph = math.cos(math.pi*(self.cycle+1)/(2*self.total_cycle))
        all_loss_ =cp.deepcopy(self.all_loss)
        all_loss_.pop('GCN')
        res = max(all_loss_, key=lambda x: self.all_loss[x])
        model = cp.deepcopy(self.models[res])
        use_cuda = torch.cuda.is_available()
        device = torch.device("cuda" if use_cuda else "cpu")
        #用于存储现有model对当前数据标签的预测结果
        predict_labels = []
        # print('1111111111111')
        #print('3333')
        ...
        for time, snapshot in enumerate(self.dataset):
            with torch.cuda.device(device=device):
                ...
                labels = torch.argmax(model(x, edge_index, edge_weight).detach(),
                                      dim=1).long().tolist()
                # loss = self.all_loss[key]
            ...
            with torch.cuda.device(device=device):
                for key, value in self.models.items():
                    if (key in ['FREE','VICTIM'])==False:
                        ...
                        edge_index = snapshot.edge_index.to(device)
                        edge_weight = snapshot.edge_attr.to(device)
                        labels = torch.argmax(self.models[key](x, edge_index, edge_weight).detach(), dim=1).long().tolist()
                        #loss = self.all_loss[key]
                        label_list.append(labels)
            predict_labels[time] = self.label_predict(label_list)
        '''
        ...
        for i in range(len(a)):
            try:
                new_a.append(s_[float(a[i])]+1)
                #new_a.append(s_[float(a[i])])
            except:
                new_a.append(0)
        ...
        new_b = [s2_[float(b[i])]+1 for i in range(len(b))]
        ...
        for i in range(self.time_span):
            ...
            neighbor_dics.append(neighbor_dic)
        unlabeled_neighbor_imp = self.temporal_neighbor_add_imp(t_labeled_set, neighbor_dics, imp)
        #median = np.percentile(unlabeled_neighbor_imp, 30)
        median = 0
        # exit()
        ...
        c = (sum(count.values())+self.budget)/self.class_num
        ...
        #data sampling start
        for i in tqdm(range(n), ncols=100):
            # get median of unlabeled_neighbor_imp
            #median = np.median(unlabeled_neighbor_imp)

            # median selection is used for guarantee low importance node selection



            #if len(np.ones(self.time_span)[budget_end_list]) ==0:
                #break
            if used_budget + qcost_min > (self.cycle+1)*n :#or p == 0
                break
            # 返回各行的和
            # l 判断循环的结束， p判断为0的时候, q判断
            l = -1
            cvalues = list(count.values())
            #print(cvalues)
            #
            if qcost[0]==1:
                p3 = 1
                ...
                p3 = 2 #0.8
                #p3 = np.std(cvalues) / (max(cvalues) - min(cvalues))

            #p3 = 2
            #print('p3 is '+str(p3))
            #stop_time = 0
            #stop_time_limitation = 9
            while l == -1:
                #p3 = self.cycle/ self.total_cycle
                q_idx_ = (unlabeled_neighbor_imp / (standard_qcost**p3)).argmax() #np.sqrt()
                q_idx = np.arange(self.labeled_set.shape[0] * self.labeled_set.shape[1])[~t_labeled_set][q_idx_]
                time = q_idx // self.node_num
                if (cost_in_time[time]+qcost_max<=self.budget ) and (cost_in_time[time]+2*qcost_min>=self.budget):
                    q_idx_ = unlabeled_neighbor_imp.argmax()  # np.sqrt()
                    q_idx = np.arange(self.labeled_set.shape[0] * self.labeled_set.shape[1])[~t_labeled_set][q_idx_]
                if unlabeled_neighbor_imp[q_idx_] == -100 or unlabeled_neighbor_imp[q_idx_] == -200:
                    break
                if used_budget + qcost[q_idx_] > (self.cycle+1)*n: #########
                    return torch.tensor(t_labeled_set).reshape(self.time_span, self.node_num).numpy()
                    #unlabeled_neighbor_imp[q_idx_] = -100
                    #stop_time += 1
                    #continue
                #if stop_time>stop_time_limitation:
                    #return torch.tensor(t_labeled_set).reshape(self.time_span, self.node_num).numpy()
                ...
                class_q = predict_labels[q_idx // self.node_num][q_idx%self.node_num]
                ...
                _end = end - np.arange(end)[t_labeled_set[0:end]].shape[0]
                #if len(np.ones(self.node_num)[t_labeled_set[start:end]].tolist()) >= self.budget \
                if unlabeled_neighbor_imp[q_idx_] < median:
                    unlabeled_neighbor_imp[q_idx_] = -100
                    continue
                elif sum(t_query_cost[start:end][t_labeled_set[start:end]]) + min(
                        t_query_cost[start:end][~t_labeled_set[start:end]]) > self.budget:
                    # unlabeled_imp[q_idx_] = 0
                    # 区间内所有imp值归0，以后不再选择
                    # imp[start:end] = torch.tensor([-100 for i in range(start, end)])
                    ...
                    cost_in_time[q_idx // self.node_num] += t_query_cost[q_idx]
                    # unlabeled_imp = np.delete(unlabeled_imp, q_idx_, 0)
                    # 每次选择多个
                    unlabeled_neighbor_imp = np.delete(unlabeled_neighbor_imp, q_idx_, 0)
                    qcost = t_query_cost[~t_labeled_set]
        ...
        for i in range(self.labeled_set.shape[0]*self.labeled_set.shape[1]):
            ...
            for key, value in self.all_loss.items():
                for key2, value2 in self.all_loss.items():
                    #print(key, value)
                    #print(key2, value2)
                    error1 = 0.5 * torch.log2(value/(1-value))
                    error2 = 0.5 * torch.log2(value2/(1-value2))
        ...
        for i in range(self.labeled_set.shape[0] * self.labeled_set.shape[1]):
            ...
            for key, value in self.all_loss.items():
                for key2, value2 in self.all_loss.items():
                    ...
                    if i < self.node_num:
                        result2 += error1*error2*torch.sqrt(
                            torch.dist(all_type_embeddings[key][i]*all_type_embeddings[key2][i],
                                       all_type_embeddings[key][i + self.node_num]*all_type_embeddings[key2][i + self.node_num]))
                        ...
                        b= torch.sqrt(
                            torch.dist(all_type_embeddings[key][i]*all_type_embeddings[key2][i],
                                       all_type_embeddings[key][i - self.node_num]*all_type_embeddings[key2][i - self.node_num]))
                        result2 += error1 * error2* torch.max(a,b)
            ...
            INF.append(float(result2))
            #INF.append(float(result+result2))
    ...
    def rep(self, model, dataset):
        ...
        Rep = 1/(1 + torch.abs(torch.tensor(
    ...
    def neighbor_add_imp_in_time(self, t_labeled_set, neighbor_dics, add_imp, imp, t):
        ...
        add_imp[removed_imp_ids] =-100
                ...
                #exit()
        return add_imp
    # output the predicted labels of current models
    def label_predict(self,label_list):
        output = []
        all_loss = list(self.all_loss.values())
        label_list = torch.transpose(torch.tensor(label_list), 0, 1)
        for i in range(label_list.shape[0]):
            total = {j:0 for j in range(len(all_loss))}
            for k in range(label_list.shape[1]):
                #print(all_loss[label_list[i][k]])
                #print(label_list[i][k])
                total[int(label_list[i][k])]+=all_loss[label_list[i][k]]
                #print(total[int(label_list[i][k])])
            output.append(max(total,key=lambda x:total[x]))
        return output
