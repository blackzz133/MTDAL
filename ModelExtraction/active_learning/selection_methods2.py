...
from ModelExtraction.active_learning.query_strategies2 import KCenterGreedy
from ModelExtraction.active_learning.query_strategies2.active_learning_temporal_graph import ALTG_strategy
from ModelExtraction.active_learning.query_strategies2.tgcn_active_learning import MTTAL_strategy
from ModelExtraction.active_learning.query_strategies2.random import Random_strategy
...
def query_samples(models, method, dataloaders, labeled_set, cycle, all_loss, args, query_cost=None, victim_labels = None):
    ...
    elif method == 'RNNPrior':
        arg = RNN_Prior(models, dataloaders, labeled_set,  args)
    elif method =='GCNPrior':
        arg = GCN_Prior(models, dataloaders, labeled_set,  args)
    elif method == 'GCNOnly':
        arg = GCN_Only(models, dataloaders, labeled_set, args)
...
def random_selection(models, dataloaders,data_unlabeled, labeled_set, cycle, args):
    return 0



def RNN_Prior(models, dataloaders,data_unlabeled, labeled_set, cycle, args):
    return 0

def GCN_Prior(models, dataloaders,data_unlabeled, labeled_set, cycle, args):
    return 0

def GCN_Only(models, dataloaders,data_unlabeled, labeled_set, cycle, args):
    return 0
...
def DEAL(models, dataloaders, labeled_set, args):
    model = models['RGCN'].to('cpu')
    dataset = dataloaders['sim_train']
    attack = DeepFool(model, steps=50, overshoot=0.02)
    for time, snapshot in enumerate(dataset):
        y_hat = torch.argmax(model(snapshot.x, snapshot.edge_index, snapshot.edge_attr).detach(), dim=1).long()[labeled_set[time]]
        adv_nodes = attack(snapshot.x, snapshot.edge_index, snapshot.edge_attr, y_hat)[labeled_set[time]]
        print(torch.tensor(snapshot.x).shape)
        print(snapshot.x)
        print(torch.tensor(adv_nodes).shape)
        print(adv_nodes)
        exit()

def DEAL_Kcenter(models, dataloaders,data_unlabeled, labeled_set, cycle, args):
    return 0
